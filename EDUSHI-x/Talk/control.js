// =========================================================
// TALKY ZONE - CORE CONTROL SCRIPT
// =========================================================
...
// Convert plain URLs in text into clickable HTML links
function linkify(inputText) {
    if (!inputText) return "";
    // If message is already raw HTML like image/video/voice tags, return as is
    if (inputText.startsWith('<img') || inputText.startsWith('<video') || inputText.startsWith('<div') || inputText.startsWith('<a')) {
        return inputText;
    }
    const urlPattern = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
    return inputText.replace(urlPattern, '<a href="$1" target="_blank" rel="noopener noreferrer">$1</a>');
}
...
async function loadGroupMessages() {
    ...
    // Set partner details (Robin gets Arya's info, Arya gets Robin's)
    const targetKey = currentUser.user_key === '894818' ? '143143' : '894818';
    const partnerMsg = reversedMsgs.find(m => m.sender_key === targetKey);

    if (partnerMsg) {
        updatePartnerHeader(partnerMsg.sender_name, partnerMsg.sender_pfp, partnerMsg.sender_role);
    } else {
        const defaultPartner = targetKey === '143143' ? 'Arya' : 'Robin';
        updatePartnerHeader(defaultPartner, 'https://via.placeholder.com/40', 'normal');
    }
...
function listenRealtimeMessages() {
    ...
    // 2. Presence Tracking (Online / Last Seen Status)
    const targetKey = currentUser.user_key === '894818' ? '143143' : '894818';
    const presenceChannel = client.channel('talky-presence', {
        config: { presence: { key: currentUser.user_key } }
    });

    presenceChannel
        .on('presence', { event: 'sync' }, () => {
            const state = presenceChannel.presenceState();
            const isTargetOnline = !!state[targetKey];
            ...
            if (isTargetOnline) {
                ...
                const lastSeenTime = localStorage.getItem(`last_seen_${targetKey}`);
                if (statusText) {
                    statusText.style.color = '#94a3b8';
                    if (lastSeenTime) {
                        const dateObj = new Date(parseInt(lastSeenTime));
                        const formattedTime = dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                        statusText.innerText = `Last seen at ${formattedTime}`;
                    } else {
        ...
        .on('presence', { event: 'leave' }, ({ key }) => {
            if (key === targetKey) {
                localStorage.setItem(`last_seen_${key}`, Date.now().toString());
            }
        })
    ...
    const msgHTML = `
        <div class="msg-row ${isMe ? 'sent' : 'received'}" id="msg-${msg.id}">
            <div class="msg-bubble" ondblclick="triggerReply('${safeSender}', '${safeText}')">
                ...
                ${replyHTML}
                <div class="msg-content">${formattedMessage}</div>
...
async function uploadAndSendFile(input) {
    ...
    if (file.type.startsWith('image/')) {
        ...
        fileHTML = `<video src="${fileUrl}" class="chat-media-preview" onclick="openMediaModal('${fileUrl}', 'video')">`;
    } else {
